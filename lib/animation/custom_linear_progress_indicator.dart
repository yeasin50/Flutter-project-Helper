...
class _CustomLinearProgressIndicatorState extends State<CustomLinearProgressIndicator> with SingleTickerProviderStateMixin {
  late AnimationController controller = AnimationController(vsync: this, duration: const Duration(seconds: 1))
    ..addListener(() {
      setState(() {});
    })
    ..repeat(reverse: true);

  late Animation animation = Tween<double>(begin: -1, end: 1).animate(controller);
    ...
    return ColoredBox(
      ...
      child: Align(
        ...
        child: Container(
          ...
          width: widget.maxProgressWidth - widget.maxProgressWidth * (animation.value as double).abs(),
...
void main(List<String> args) {
  runApp(MaterialApp(
    home: Scaffold(
      appBar: AppBar(
        title: const Text('CustomLinearProgressIndicator Test'),
      ),
      body: Container(
